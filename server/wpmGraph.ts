...
import { Pool, RowDataPacket } from 'mysql2/promise'; // Or your database library's types (e.g., pg for PostgreSQL)
...
const calculateAverageWPM = async (pool: Pool): Promise<number | null> => {
    try {
      ...
      if (rows.length > 0 && rows[0].average_wpm !== null) {
        return parseFloat(rows[0].average_wpm.toString()); // Ensure it's a number
      } else {
